...
9. **C2 — Vault unlock:** Re-enter recovery code **every new login/session** (safer). Code stays long (high entropy). Store it as a **Secure Note** (not as the PractisBase website password). Vault forms use `type="text"` + ignore attributes so browsers/password managers do **not** offer to save/update the site login password.
...
### ROADMAP STATUS: LOCKED — Phase 0–5 shipped; closed beta before Stripe (#18); revisions #13–21 applied
