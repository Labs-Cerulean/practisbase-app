...
9. **C2 — Vault unlock:** Re-enter recovery code **every new login/session** (safer). Code stays long (high entropy); daily UX relies on password manager autofill under a distinct “PractisBase Medical Vault” credential — never overwrite the account login password. Forms use separate autocomplete identities so managers treat login and vault code as different keys.
