...
@section('content')
    <div style="max-width: 760px; margin: 0 auto;">
        ...
        <h1 style="margin: 0.4rem 0 1rem; color: var(--primary-navy); font-size: 1.4rem;">{{ $template ? 'Edit template' : 'New journal template' }}</h1>
        ...
        <form action="{{ $template ? '/pro/medical/templates/'.$template->id : '/pro/medical/templates' }}" method="POST" id="template-builder-form">
            ...
            <div style="background: white; border: 1px solid var(--border-light); border-radius: var(--radius-lg); padding: 1.25rem; margin-bottom: 1rem; box-shadow: var(--shadow-sm);">
                ...
                <div style="font-size: 0.75rem; color: var(--text-muted); margin-top: 0.75rem; line-height: 1.4;">
                    Drag order top → bottom is the form order. Keys are auto-generated from labels if left blank.
                </div>
            </div>
...
(function () {
    ...
    function rowHtml(field, index) {
        return '' +
            '<div class="tpl-field-row" draggable="true" style="display:grid; grid-template-columns: 1.4fr 1fr 90px auto; gap:0.5rem; align-items:end; padding:0.75rem; background:#f8fafc; border:1px solid var(--border-light); border-radius:var(--radius-md);">' +
            '<div><label style="display:block;font-size:0.7rem;font-weight:700;color:var(--text-muted);margin-bottom:0.25rem;">Label</label>' +
            '<input type="text" name="fields[' + index + '][label]" value="' + escapeAttr(field.label || '') + '" required maxlength="80" placeholder="e.g. LMP" style="width:100%;padding:0.55rem;border:1px solid var(--border-light);border-radius:var(--radius-md);"></div>' +
            '<div><label style="display:block;font-size:0.7rem;font-weight:700;color:var(--text-muted);margin-bottom:0.25rem;">Key (optional)</label>' +
            '<input type="text" name="fields[' + index + '][key]" value="' + escapeAttr(field.key || '') + '" maxlength="40" placeholder="auto" style="width:100%;padding:0.55rem;border:1px solid var(--border-light);border-radius:var(--radius-md);font-family:ui-monospace,monospace;font-size:0.85rem;"></div>' +
