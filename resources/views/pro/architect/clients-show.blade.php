...
@section('content')
    <div style="display: flex; justify-content: space-between; align-items: flex-start; gap: 1rem; flex-wrap: wrap; margin-bottom: 1.25rem;">
        <div>
            <a href="/pro/architect/clients" style="color: var(--text-muted); text-decoration: none; font-weight: 600;">← Clients</a>
        ...
        <div style="display: flex; gap: 0.5rem; flex-wrap: wrap;">
            <a href="/pro/architect/clients/{{ $client->id }}/edit" style="background: white; border: 1px solid var(--border-light); color: var(--primary-navy); padding: 0.55rem 1rem; border-radius: var(--radius-md); font-weight: 600; text-decoration: none;">Edit</a>
            <a href="/pro/architect/projects/create?client_id={{ $client->id }}" style="background: #3f6212; color: white; padding: 0.55rem 1rem; border-radius: var(--radius-md); font-weight: 600; text-decoration: none;">+ Project</a>
            <a href="/pro/architect/documents/create?client_id={{ $client->id }}" style="background: white; border: 1px solid var(--border-light); color: var(--primary-navy); padding: 0.55rem 1rem; border-radius: var(--radius-md); font-weight: 600; text-decoration: none;">+ Document</a>
        </div>
    </div>
    ...
    <div style="display: grid; grid-template-columns: 1.4fr 1fr; gap: 1.25rem;" class="arch-split">
        <div>
            ...
            @else
                <div style="display: grid; gap: 0.65rem;">
                    @foreach($client->projects as $project)
                        <a href="/pro/architect/projects/{{ $project->id }}" style="display: block; background: white; border: 1px solid var(--border-light); border-radius: var(--radius-md); padding: 0.95rem 1.1rem; text-decoration: none; box-shadow: var(--shadow-sm);">
            ...
            @else
                <div style="display: grid; gap: 0.5rem;">
                    @foreach($client->documents as $doc)
                        <a href="/pro/architect/documents/{{ $doc->id }}" style="display: block; background: white; border: 1px solid var(--border-light); border-radius: var(--radius-md); padding: 0.75rem 0.9rem; text-decoration: none;">
