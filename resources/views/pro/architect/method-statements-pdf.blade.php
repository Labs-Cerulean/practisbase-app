...
<body>
    ...
    <div class="banner">Architect method statement · issued copy</div>
    <div class="meta">
        {{ $statement->typeLabel() }}
    ...
    <table class="grid">
        <tr>
            <td style="width: 50%;">
                ...
                <div class="value"><strong>{{ $statement->client_name ?: '—' }}</strong></div>
                @if($statement->client_address)<div class="value">{{ $statement->client_address }}</div>@endif
                @if($statement->project_description)
                    <div class="label" style="margin-top: 8px;">Project description</div>
                    <div class="value" style="white-space: pre-wrap;">{{ $statement->project_description }}</div>
                @endif
            </td>
            <td style="width: 50%;">
                <div class="label">Site</div>
                <div class="value">{{ $statement->site_address ?: '—' }}</div>
                @if($statement->paApplication)
                    <div class="label" style="margin-top: 8px;">PA</div>
                    <div class="value">{{ $statement->paApplication->displayLabel() }}</div>
                @endif
                @if($statement->project)
                    <div class="label" style="margin-top: 8px;">Project</div>
                    <div class="value">{{ $statement->project->name }}</div>
                @endif
            </td>
        </tr>
    </table>
    ...
    @if($payload['appendix_ref'])
        <h2>Annexes / appendices</h2>
        <p class="section-body">{{ $payload['appendix_ref'] }}</p>
    @endif
        ...
        <table style="width: 100%; border-collapse: collapse;">
            <tr>
                <td style="width: 55%;">
                    ...
                    <div class="auth-code">{{ $statement->issue_code }}</div>
                </td>
                <td>
                    <div class="label">Stamped</div>
                    <div style="font-size: 13px; font-weight: bold;">{{ optional($statement->stamped_at)->format('d M Y H:i') }}</div>
                </td>
            </tr>
        </table>
    </div>
