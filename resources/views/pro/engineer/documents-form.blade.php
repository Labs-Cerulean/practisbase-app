...
@section('content')
    <div style="margin-bottom: 1.25rem;">
        <a href="/pro/engineer/documents" style="color: var(--text-muted); text-decoration: none; font-weight: 600;">← Documents</a>
        <h1 style="margin: 0.5rem 0 0; color: var(--primary-navy); font-size: 1.5rem;">Upload document</h1>
        <p style="margin: 0.35rem 0 0; color: var(--text-muted); font-size: 0.88rem;">Attach at client, project, or PA. First file becomes Rev 1.</p>
    </div>
    ...
    <form method="POST" action="/pro/engineer/documents" enctype="multipart/form-data" style="background: white; border: 1px solid var(--border-light); border-radius: var(--radius-lg); padding: 1.35rem; max-width: 760px; box-shadow: var(--shadow-sm);">
        @csrf
        <div style="display: grid; gap: 0.85rem;">
                ...
                <div>
                    ...
                    <select name="category" style="width: 100%; padding: 0.65rem 0.75rem; border: 1px solid var(--border-light); border-radius: var(--radius-md);">
                        @foreach($categories as $key => $label)
                            <option value="{{ $key }}" @selected(old('category', 'drawing') === $key)>{{ $label }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
            ...
            <div id="scopeClient">
                ...
                <select name="engineer_client_id" style="width: 100%; padding: 0.65rem 0.75rem; border: 1px solid var(--border-light); border-radius: var(--radius-md);">
                    <option value="">Select…</option>
                    @foreach($clients as $c)
                        <option value="{{ $c->id }}" @selected(old('engineer_client_id', $prefill['client_id'] ?? '') == $c->id)>{{ $c->name }}</option>
                    @endforeach
                </select>
            </div>
            <div id="scopeProject">
            ...
            <div id="scopePa">
                ...
                <select name="engineer_pa_application_id" style="width: 100%; padding: 0.65rem 0.75rem; border: 1px solid var(--border-light); border-radius: var(--radius-md);">
                    <option value="">Select…</option>
                    @foreach($pas as $pa)
                        <option value="{{ $pa->id }}" @selected(old('engineer_pa_application_id', $prefill['pa_id'] ?? '') == $pa->id)>{{ $pa->displayLabel() }}@if($pa->project) · {{ $pa->project->name }}@endif</option>
                    @endforeach
                </select>
            </div>
            ...
            <div>
                ...
                <div style="font-size: 0.75rem; color: var(--text-muted); margin-top: 0.25rem;">Up to 20 MB. Drawings, PDFs, photos, Word, calcs.</div>
            </div>
            <div>
            ...
            <button type="submit" style="background: var(--primary-cerulean); color: white; border: none; padding: 0.75rem 1.1rem; border-radius: var(--radius-md); font-weight: 700; cursor: pointer; width: fit-content;">Upload as Rev 1</button>
        </div>
    </form>
