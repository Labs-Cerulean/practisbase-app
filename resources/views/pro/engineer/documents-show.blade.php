    ...
    <div style="display: flex; justify-content: space-between; align-items: flex-start; gap: 1rem; flex-wrap: wrap; margin-bottom: 1.25rem;">
        <div>
            ...
            <p style="margin: 0; color: var(--text-muted); font-size: 0.9rem;">
                {{ $document->scopeLabel() }}
                @if($document->paApplication) · {{ $document->paApplication->displayLabel() }}
        ...
        <section style="background: white; border: 1px solid var(--border-light); border-radius: var(--radius-lg); padding: 1.2rem; box-shadow: var(--shadow-sm);">
            ...
            @forelse($document->revisions as $rev)
                <div style="display: flex; justify-content: space-between; gap: 1rem; padding: 0.7rem 0; border-bottom: 1px dashed var(--border-light);">
                    ...
                    <a href="/pro/engineer/documents/{{ $document->id }}/revisions/{{ $rev->id }}/download" style="font-weight: 700; color: var(--primary-cerulean); text-decoration: none; font-size: 0.85rem;">Open</a>
                </div>
            @empty
                <p style="color: var(--text-muted);">No files yet.</p>
            @endforelse

            <form method="POST" action="/pro/engineer/documents/{{ $document->id }}/revisions" enctype="multipart/form-data" style="margin-top: 1rem; display: grid; gap: 0.65rem; border-top: 1px solid #e2e8f0; padding-top: 0.9rem;">
                @csrf
                ...
                <button type="submit" style="background: var(--primary-cerulean); color: white; border: none; padding: 0.6rem 0.9rem; border-radius: var(--radius-md); font-weight: 700; cursor: pointer; width: fit-content;">Upload revision</button>
            </form>
        </section>

        <section style="background: white; border: 1px solid var(--border-light); border-radius: var(--radius-lg); padding: 1.2rem; box-shadow: var(--shadow-sm);">
            <h2 style="margin: 0 0 0.75rem; font-size: 1.05rem; color: var(--primary-navy);">Details</h2>
            <form method="POST" action="/pro/engineer/documents/{{ $document->id }}" style="display: grid; gap: 0.65rem;">
                @csrf
                @method('PUT')
