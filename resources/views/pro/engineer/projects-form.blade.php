        ...
        <form method="POST" action="{{ $action }}" style="background: white; border: 1px solid var(--border-light); border-radius: var(--radius-lg); padding: 1.35rem; box-shadow: var(--shadow-sm); max-width: 820px;">
            ...
            <div style="display: grid; gap: 0.85rem;">
                <div>
                    ...
                    <select name="engineer_client_id" required style="width: 100%; padding: 0.65rem 0.75rem; border: 1px solid var(--border-light); border-radius: var(--radius-md);">
                        @foreach($clients as $c)
                            <option value="{{ $c->id }}" @selected(old('engineer_client_id', $preselectClientId) == $c->id)>{{ $c->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
