...
@section('content')
    ...
    @if($clients->isEmpty())
        <div style="padding: 3rem; border: 2px dashed var(--border-light); border-radius: var(--radius-md); text-align: center;">
            <p style="color: var(--text-muted); margin-bottom: 1rem;">No clients found.</p>
            <a href="/clients/create" style="color: var(--primary-cerulean); font-weight: 600;">Add a new client &rarr;</a>
        </div>
    @else
        <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(320px, 1fr)); gap: 1.5rem;">
            @foreach($clients as $client)
                <div style="background: white; border: 1px solid var(--border-light); border-radius: var(--radius-md); padding: 1.5rem; box-shadow: var(--shadow-sm); display: flex; flex-direction: column; position: relative;">
                    ...
                    <div style="display: flex; gap: 0.5rem;">
                        ...
                        @endif
                        <a href="/clients/{{ $client->id }}" style="flex: 1; text-align: center; padding: 0.5rem; background: rgba(2, 132, 199, 0.1); color: var(--primary-cerulean); border-radius: 6px; font-weight: 600; font-size: 0.85rem; text-decoration: none;">View Profile</a>
                    </div>
                </div>
            @endforeach
        </div>
